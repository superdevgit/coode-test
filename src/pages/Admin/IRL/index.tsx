...
import { Metaplex, keypairIdentity, bundlrStorage, MetaplexFile, BundlrStorageDriver, walletAdapterIdentity } from "@metaplex-foundation/js";
import {
  Connection,
  Keypair,
  Commitment,
...
const IRLCreate = () => {
  ...
  const handleNFTMint = async () => {
    try {
      ...
      const validation = IRLCreatealidation(irlCreate);
      if (!validation) return
      setLoading(true)

      const connection = new Connection(clusterApiUrl(CONFIG.SOLANA_NETWORK));
      const metaplex = Metaplex.make(connection, { cluster: CONFIG.SOLANA_NETWORK })
      ...
      payload.append('image', irlCreate.image)
        ...
        <div className="border-4 mt-6 md:px-8 px-4 pt-8 pb-14 border-[#606060] bg-[#60606040] rounded-[0.7rem]">
          <div className="md:flex block justify-between items-start">
            <div className="basis-[48%] dark:bg-jacarta-700 dark:border-jacarta-600 border-jacarta-100 group relative flex max-w-md flex-col items-center justify-center rounded-lg border-2 border-dashed bg-white py-20 px-5 text-center">
              ...
              {/* <input type="file" onChange={(e: any) => setIrlCreate({ ...irlCreate, image: e.target.file[0] })} /> */}
